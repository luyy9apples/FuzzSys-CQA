...
param_fn = f'./params/{dataset}_QTO_params.yml'
...
ys_smooth = splev(bin_x, tck)
ys_all_smooth = splev(ts_all, tck)
ys_smooth = np.clip(ys_smooth, 0, 1)
ys_all_smooth = np.clip(ys_all_smooth, 0, 1)

pos_mask = (bin_x >= tck[0][-1]).astype(float)
...
ax2.plot(ts_all, ys_all_smooth, color='#CF5F55', linewidth=2.0, linestyle='--', label=r'$\mu_S(\lambda)$')
...
save_fig = f'img/{dataset}_QTO_{bins_num}_4.7_valid.svg'
