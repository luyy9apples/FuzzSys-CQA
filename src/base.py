...
from src.util import query_to_atoms
import src.discrete as d1
import src.discrete_cqd as d2
import src.discrete_qto as d3
...
class CQD(nn.Module):
    ...
    def bce_loss(self, pred, target):
        ...
        #     with torch.no_grad():
        #         logit = pred[is_negative] / adversarial_temperature
    ...
    def loss(self,
        ...
        if self.use_ce:
            ...
            num_entity = scores_o.shape[1]
            target_o = F.one_hot(triples[:, 2], num_classes=num_entity).cuda()
            target_s = F.one_hot(triples[:, 0], num_classes=num_entity).cuda()
            
            l_fit = self.bce_loss(torch.sigmoid(scores_o), target_o.float()) + self.bce_loss(torch.sigmoid(scores_s), target_s.float())
    ...
    def forward(self,
        ...
        for query_structure, queries in batch_queries_dict.items():
            ...
            if 'continuous' in self.method:
                ...
                if self.wo_fl:
                    if graph_type == "1p":
                        scores = d1.query_1p(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function)
                        
                    elif graph_type == "2p":
                        scores = d1.query_2p(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function,
                        ...
                        scores = d1.query_3p(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function,
                                            k=self.k, t_norm=t_norm)
                        ...
                        scores = d1.query_2i(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function, t_norm=t_norm)

                    elif graph_type == "3i":
                        scores = d1.query_3i(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function, t_norm=t_norm)
                        
                    elif graph_type == "pi":
                        scores = d1.query_pi(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function,
                                            k=self.k, t_norm=t_norm)
                        ...
                        scores = d1.query_ip(entity_embeddings=self.embeddings[0],
                                            predicate_embeddings=self.embeddings[1],
                                            queries=queries,
                                            scoring_function=scoring_function,
                                            k=self.k, t_norm=t_norm)
                        ...
                        scores = d1.query_2u_dnf(entity_embeddings=self.embeddings[0],
                                                predicate_embeddings=self.embeddings[1],
                                                queries=queries,
                                                scoring_function=scoring_function,
                                                t_conorm=t_conorm)
                        ...
                        scores = d1.query_up_dnf(entity_embeddings=self.embeddings[0],
                                                predicate_embeddings=self.embeddings[1],
                                                queries=queries,
                                                scoring_function=scoring_function,
                ...
                if not self.wo_fl and self.backbone_type == 'CQD': # Fuzzy System
                    if graph_type == "1p":
                ...
                if not self.wo_fl and self.backbone_type == 'QTO': # Fuzzy System
                    if graph_type == "1p":
                        scores = d3.query_1p(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "2p":
                        scores = d3.query_2p(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "3p":
                        scores = d3.query_3p(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "2i":
                        scores = d3.query_2i(entity_embeddings=self.embeddings[0], queries=queries)

                    elif graph_type == "2in":
                        scores = d3.query_2in(entity_embeddings=self.embeddings[0], queries=queries)

                    elif graph_type == "3i":
                        scores = d3.query_3i(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "3in":
                        scores = d3.query_3in(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "pi":
                        scores = d3.query_pi(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "pin":
                        scores = d3.query_pin(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "ip":
                        scores = d3.query_ip(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "inp":
                        scores = d3.query_inp(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "pni":
                        scores = d3.query_pni(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "2u-DNF":
                        scores = d3.query_2u_dnf(entity_embeddings=self.embeddings[0], queries=queries)
                        
                    elif graph_type == "up-DNF":
                        scores = d3.query_up_dnf(entity_embeddings=self.embeddings[0], queries=queries)
