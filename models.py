...
class KGReasoning(nn.Module):
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        if isinstance(model, CQD):
            ...
            positive_batch = torch.cat((input_batch, positive_sample.unsqueeze(1)), dim=1)

            loss = model.loss(positive_batch)
    ...
    def test_step(model, easy_answers, hard_answers, args, test_dataloader, query_name_dict, save_result=False, save_str="", save_empty=False):
        ...
        if args.do_valid and args.do_save:
            tmp_fn = f'./params/{args.dataname}_{args.backbone_type}_1p_{args.threshd}_validation.txt'
        ...
        with torch.set_grad_enabled(requires_grad):
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                for idx, (i, query, query_structure) in enumerate(zip(argsort[:, 0], queries_unflatten, query_structures)):
                    ...
                    #### 1p valid ####
                    if args.do_valid and args.do_save:
                        ans_list = list(easy_answer) + list(hard_answer)
                        cur_score = negative_logit[idx, ans_list].cpu().numpy()
                        cur_score = cur_score[cur_score > args.threshd]
                        cur_score_str = '\n'.join(['1,'+str(s) for s in cur_score])
                        tmp_file.write(cur_score_str+'\n')

                        n_idx = set(np.arange(negative_logit.shape[1])) - set(ans_list)
                        n_score = negative_logit[idx, list(n_idx)].cpu().numpy()
                        n_score = n_score[n_score > args.threshd]
                        if len(cur_score) > 0 and len(n_score) > 0:
                            n_score = np.random.choice(n_score, len(cur_score))
                            n_score_str = '\n'.join(['0,'+str(s) for s in n_score])
                            tmp_file.write(n_score_str+'\n')
