...
@torch.no_grad()
def neural_adj_matrix(model, rel, nentity, device, thrshd, adj_list, num_scale=1.0):
    bsz = 100
    ...
    for (h, t) in adj_list:
        num[h, 0] += 1
    num = torch.maximum(num * num_scale, torch.ones(nentity, 1).to(torch.float).to(device))
...
class KGReasoning(nn.Module):
    def __init__(self, args, device, adj_list, query_name_dict, name_answer_dict):
        ...
        if args.data_path.split('/')[-1].split('-')[1] == "237":
            dataset_name += "-237"
        filename = 'neural_adj/'+dataset_name+'_'+str(args.fraction)+'_'+str(args.thrshd)+'_'+str(args.num_scale)+'.pt'
        if os.path.exists(filename):
            ...
            for i in tqdm(range(args.nrelation)):
                relation_embedding = neural_adj_matrix(kbc_model, i, args.nentity, device, args.thrshd, adj_list[i], args.num_scale)
